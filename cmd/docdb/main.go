...
// Package main is the entry point for DocDB server.
package main
...
var cli struct {
	...
	Ping struct{} `cmd:"" help:"Ping existing DocDB instance."`
	...
	OTel struct {
		ServiceName string `default:"docdb" help:"OpenTelemetry service name."`
		Traces      struct {
	...
	Dev struct {
		...
		Telemetry struct {
			URL            string        `default:"https://beacon.docdb.hanzo.ai/" hidden:""`
...
var (
	...
	kongOptions = []kong.Option{
		kong.Vars{
			...
			"help_telemetry":  "Enable or disable basic telemetry reporting. See https://beacon.docdb.hanzo.ai.",
		},
		kong.DefaultEnvars("DOCDB"),
	}
)
...
// It also replaces "-" with "" for some flags to make it easier to configure DocDB with environment variables.
...
// run sets up environment based on provided flags and runs DocDB.
func run() {
	...
	logger.LogAttrs(context.Background(), slog.LevelInfo, "Starting DocDB "+info.Version, startupFields...)
