...
# for production releases (`docdb` image)
...
FROM --platform=$BUILDPLATFORM golang:1.26.5-bookworm AS production-prepare
...
# TODO https://github.com/hanzoai/docdb/issues/5449
FROM golang:1.26.5-bookworm AS production-build
ENV GOTOOLCHAIN=auto
...
COPY --from=production-build /src/bin/docdb /docdb
...
COPY build/docdb/passwd /etc/passwd
COPY build/docdb/group  /etc/group
USER docdb:docdb

COPY --from=production-build /src/bin/docdb /docdb
COPY --from=production-build --chown=docdb:docdb /state /state
...
LABEL org.opencontainers.image.title="DocDB"
